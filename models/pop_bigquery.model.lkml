...
include: "/views/pop_views_donottouch/*.view.lkml"
include: "/views/pop_views_to_edit/*.view.lkml"
...
explore: flexible_pop {
  ...
  always_filter: {
    filters: [pop.date_filter: "last 999 weeks", pop.within_period_type: "week", pop.over_period_type: "year"]
  }
...
# Rename (& optionally repeat) below join to match your pop view(s)
  ...
  join: pop_order_items_delivered {
    ...
    relationship: many_to_one
    #Apply join name below in sql_on
    sql_on: DATE(pop_order_items_delivered.join_date) = DATE_TRUNC(
                      DATE_ADD(
                          DATE_ADD(
                            DATE({% date_end pop.date_filter %}), INTERVAL 0 - ${within_periods.n} {% parameter pop.within_period_type %}
                          ), INTERVAL 0 - ${over_periods.n} {% parameter pop.over_period_type %}
